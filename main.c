...
void drawFramesPerSecond(void)
{
	...
	glRasterPos4f(6.5, 9.3, -25.0, 2.0);
...
static void drawScene(void)
{
	...
	if (gGameState)
	{
		...
		if (gGameWinner != NO_CHARACTER)
		{
			...
			if (!gNetworkConnection || gNetworkConnection->type != NETWORK_CLIENT_TYPE)
			{
				...
				drawString(5.0, 1.0, "Fire to play again or Escape to quit");
	...
	if (!gGameState)
	{
		...
		glColor4f(0.3, 0.2, 1.0, 0.35);
		glTranslatef(-1.0, 27.0, -100.0);
		...
		{
			glTranslatef(-1.0, 50.0, -280.0);

			glColor4f(0.3, 0.2, 1.0, 0.6);
	...
	switch (event->type)
	{
		case SDL_KEYDOWN:
				...
				&& event->key.keysym.sym != SDLK_UNKNOWN)
			{
				if (gConsoleActivated)
				{
					writeConsoleText(event->key.keysym.unicode);
				}
				else if (gNetworkAddressFieldIsActive)
				{
					writeNetworkAddressText(event->key.keysym.unicode);
				}
				else if (gNetworkUserNameFieldIsActive)
				{
					writeNetworkUserNameText(event->key.keysym.unicode);
				}
			}
