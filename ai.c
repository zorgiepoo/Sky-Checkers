...
static void directCharacterBasedOnCollisions(Character *character, int currentTime)
{
	...
	Tile *tile = &gTiles[getTileIndexLocation(character->x, character->y)];
	// gray color red == 0.31
...
static void shootWeaponProjectile(Character *character, int currentTime)
{
	...
	int tileIndex = getTileIndexLocation(character->x, character->y);
	
	// AI should be worrying about not colliding into another character
	if (tileIndex == getTileIndexLocation(characterB->x, characterB->y) ||
		tileIndex == getTileIndexLocation(characterC->x, characterC->y) ||
		tileIndex == getTileIndexLocation(characterD->x, characterD->y))
	{
		return;
	}
...
{	
	...
	tileIndex = getTileIndexLocation(character->x, character->y);
