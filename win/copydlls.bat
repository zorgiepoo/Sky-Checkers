
xcopy /y /d "%~1\SDL2-2.0.12\lib\x64\SDL2.dll" "%~2"


xcopy /y /d "%~1\SDL2_mixer-2.0.4\lib\x64\SDL2_mixer.dll" "%~2"


xcopy /y /d "%~1\SDL2_mixer-2.0.4\lib\x64\libogg-0.dll" "%~2"


xcopy /y /d "%~1\SDL2_mixer-2.0.4\lib\x64\libvorbis-0.dll" "%~2"


xcopy /y /d "%~1\SDL2_mixer-2.0.4\lib\x64\libvorbisfile-3.dll" "%~2"


xcopy /y /d "%~1\SDL2_mixer-2.0.4\lib\x64\LICENSE.ogg-vorbis.txt" "%~2"


xcopy /y /d "%~1\SDL2_ttf-2.0.14\lib\x64\SDL2_ttf.dll" "%~2"


xcopy /y /d "%~1\SDL2_ttf-2.0.14\lib\x64\libfreetype-6.dll" "%~2"

xcopy /y /d "%~1\SDL2_ttf-2.0.14\lib\x64\zlib1.dll" "%~2"


xcopy /y /d "%~1\SDL2_ttf-2.0.14\lib\x64\LICENSE.freetype.txt" "%~2"
