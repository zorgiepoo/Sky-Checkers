...
// in milliseconds
static const float TIMER_INTERVAL =				16.0;

static const float TILE_FALLING_SPEED =			1.8;
...
static float gSecondTimer =						0.0;
static float gLastSecond =						0.0;
...
static Uint32 timedEvents(Uint32 interval, void* param)
{
	...
	gSecondTimer += TIMER_INTERVAL / 1000;
	...
	static const float BEGIN_TILE_LAYER_ANIMATION =	100;
	static const float END_TILE_LAYER_ANIMATION =	200;
...
static void animateWeapAndTiles(Character *player)
{
	if (player->weap->animationState)
	{
		...
		static const float WEAPON_PROJECTILE_SPEED = 0.8;
		...
		if (!player->coloredTiles)
		{
			...
			player->player_loc = getTileIndexLocation(player->x, player->y);
...
static void firstTileLayerAnimation(int beginAnimating, int endAnimating)
{
	...
	if (gLayerColorIndex != 0 && gFirstLayerAnimationTimer > beginAnimating)
	{
			...
			gTiles[gTilesLayer[gLayerColorIndex]].blue == gTiles[gTilesLayer[gLayerColorIndex]].d_blue)
		{
			...
			gTiles[gTilesLayer[gLayerColorIndex]].red = 0.31;
			gTiles[gTilesLayer[gLayerColorIndex]].green = 0.33;
			gTiles[gTilesLayer[gLayerColorIndex]].blue = 0.36;
...
static void secondTileLayerAnimation(int beginAnimating, int endAnimating)
{
	...
	if (gLayerTwoColorIndex != 0 && gSecondLayerAnimationTimer > beginAnimating)
	{
		...
		{
			// RGB { 0.31, 0.33, 0.36 } == Grayish color
			gTiles[gTilesLayer[gLayerTwoColorIndex]].red = 0.31;
			gTiles[gTilesLayer[gLayerTwoColorIndex]].green = 0.33;
			gTiles[gTilesLayer[gLayerTwoColorIndex]].blue = 0.36;
...
void decideWhetherToMakeAPlayerAWinner(Character *player)
{
	...
	if (player->lives == 0)
	{
		...
		else if (numPlayersAlive == 1)
		{
			...
			SDL_bool winnerIsReadyForDeath = (gTiles[getTileIndexLocation(winnerCharacter->x, winnerCharacter->y)].z != -25.0 && winnerCharacter->z >= -70.0 && winnerCharacter->lives == 1);
...
static void killCharacter(Input *characterInput)
{
	...
	int location = getTileIndexLocation(player->x, player->y);
	...
	if (gTiles[location].z != -25.0 && player->z >= -70.0)
	{
		...
		static const float CHARACTER_FALLING_SPEED = 0.45;
